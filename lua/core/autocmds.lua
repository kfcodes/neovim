-- Central location for your autocommand groups and events.
-- Each augroup isolates related autocmds for clarity and easier debugging.
...
-- Filetype-specific settings: e.g.,
--     autocmd('FileType', { pattern = 'markdown', command = 'setlocal wrap' })
-- Highlight LSP diagnostics on CursorHold:
--     autocmd('CursorHold', { callback = vim.diagnostic.open_float })
-- Toggle spell-check in text files:
--     autocmd('FileType', { pattern = {'gitcommit','markdown','text'}, command = 'setlocal spell' })
